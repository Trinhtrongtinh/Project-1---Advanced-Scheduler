  ...
  if (thread_mlfqs)
    {
      ...
      t->base_priority = cur->base_priority;
...
thread_set_priority (int new_priority) 
{
  ...
  old_level = intr_disable ();
  if (thread_current ()->base_priority == thread_current ()->base_priority
      || new_priority > thread_current ()->base_priority)
...
thread_get_priority (void) 
{
  ...
  old_level = intr_disable ();
  priority = thread_current ()->base_priority;
  intr_set_level (old_level);
...
{    
  ...
  while (thread != NULL && nesting < PRI_MAX)
    {
      if (thread->status == THREAD_BLOCKED)
        {
          maybe_raise_priority (thread, thread_current ()->base_priority);
      ...
      if (lock == owned_lock)
      ...
      else
        {
          ...
          if (waiting_thread != NULL
              && waiting_thread->base_priority > highest_waiting_priority)
            highest_waiting_priority = waiting_thread->base_priority;
          e = list_next (e);
        }
    }
...
update_priority (struct thread *t)
{
  ...
  t->base_priority = trim_priority (FP_TO_INT(INT_TO_FP(PRI_MAX)
                                         - t->recent_cpu / 4
                                         - INT_TO_FP(t->nice * 2)));
}
...
{
  ASSERT (priority <= thread->base_priority);
  
  if (priority < thread->base_priority)
    thread->base_priority = thread->base_priority;
  else
    thread->base_priority = priority;
}
